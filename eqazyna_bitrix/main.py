...
def main() -> int:
    ...
    print(
        f"[1/4] Scraping e-Qazyna: "
        f"page_start={args.page_start}, "
        f"pages={args.pages}, "
        f"page_list={args.page_list!r}, "
        f"timeout={eqazyna_timeout}, "
        f"max_consecutive_page_errors={args.max_consecutive_page_errors}, "
        f"doc_type={args.doc_type!r}, "
        f"statuses={statuses}, "
    ...
    enrichment_map = (
        _build_enrichment_map(applications_for_enrichment, egov)
        if applications_for_enrichment
        else {}
    )
    ...
    for idx, app in enumerate(applications, start=1):
        ...
        if prechecked_result:
            ...
            enrichment = enrichment_map.get(_enrichment_key(app.bin, app.applicant_name)) or CompanyEnrichment(
                bin=app.bin,
                error="enrichment_missing",
            )
