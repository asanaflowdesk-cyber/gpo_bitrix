...
def check_local_python_module_references() -> None:
    """Catch stale tests/scripts that import removed project modules."""
    ...
    pattern = re.compile(
        r"(?:from\s+|import\s+)(eqazyna_bitrix(?:\.[A-Za-z_][A-Za-z0-9_]*)*)"
    )
    ...
    for module_name in sorted(module_names):
        try:
            ...
            fail(
                f"Python file references broken project module {module_name}: {exc}"
            )
