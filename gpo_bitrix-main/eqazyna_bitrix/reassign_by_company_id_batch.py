...
class Bitrix:
    ...
    def list_direct_company_contact_ids(self, company_id: str, max_pages: int) -> Set[str]:
        ...
        # Multi-company bindings. This is the authoritative relation method.
        # Do not silently continue on failure: a partial package transfer is worse
        # than a failed workflow because the user expects every linked contact.
        ...
        # Primary COMPANY_ID binding additionally catches older/legacy records.
        params: List[Tuple[str, Any]] = [("order[ID]", "ASC"), ("filter[COMPANY_ID]", company_id)]
    ...
    def list_deal_contact_ids(self, deal_ids: Sequence[str]) -> Set[str]:
        ...
        for part in chunked(clean_ids, 50):
            ...
            if batch_errors:
                details = "; ".join(
                    f"deal #{deal_id}: {batch_errors.get(f'd{index}')}"
                    for index, deal_id in enumerate(part, start=1)
                    if f"d{index}" in batch_errors
                )
                raise BitrixError(f"Failed to read contacts for company deals: {details}")
            for index, deal_id in enumerate(part, start=1):
                key = f"d{index}"
